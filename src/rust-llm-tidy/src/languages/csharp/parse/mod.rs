...
//! # Children
//!
//! - `call_targets`: call-target naming without receiver typing.
//!
//! # Re-exports
//!
...
/// `" Summary."`).
pub(crate) fn doc_comment_texts(node: tree_sitter::Node<'_>, source: &str) -> Vec<String> {
...
pub(crate) fn doc_run_start_line(node: tree_sitter::Node<'_>, source: &str) -> Option<usize> {
...
/// suffix stripped.
pub(crate) fn has_test_marker(node: tree_sitter::Node<'_>, source: &str) -> bool {
    let mut cursor = node.walk();
...
/// both.
pub(crate) fn member_kind(kind: &str) -> ItemKind {
    match kind {
...
pub(crate) fn parameter_names(node: tree_sitter::Node<'_>, source: &str) -> Vec<String> {
...
pub(crate) fn visibility_of(node: tree_sitter::Node<'_>, source: &str) -> Option<VisibilityTier> {
