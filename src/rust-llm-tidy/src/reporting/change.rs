...
///
/// The pairs come straight from [`fix_links`], so
/// no diff or line tracking is needed here; records carry no line (`None`).
///
/// [`fix_links`]: crate::rules::transform::fix_links
...
/// line:
///
/// - one per top-level move, from the reorder module's `ReorderMove`
...
mod tests {
    ...
    fn fence_changes_maps_one_record_per_anchor() {
        let anchors = vec![crate::rules::transform::FixAnchor {
            line: 7,
            kind: crate::rules::transform::FixKind::Fence,
        }];
