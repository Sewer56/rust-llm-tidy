...
count << $list
set spanned "starts
# not a comment: this spanned string payload line runs well past the
# eighty character budget and pools past the paragraph budget when
# joined, so a scanner that mistook payload for comments would fire
    ends"
