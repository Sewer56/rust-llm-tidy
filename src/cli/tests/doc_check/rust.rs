...
use super::{
    assert_has_diagnostic, binary, run_command, run_rust_fixture, rust_fixture_dir, temp_dir,
    temp_file,
};
use std::fs;
...
fn doc008_sorted_not_flagged() {
    let (stderr, _exit) = run_rust_fixture("doc008_error_variant_order.rs");
    assert!(
        !stderr.contains("save"),
...
fn doc008_variants_out_of_order() {
    let (stderr, exit) = run_rust_fixture("doc008_error_variant_order.rs");
