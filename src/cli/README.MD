...
| `--lint-scope all\|changed-lines` | Override all lint reporting scopes                              |
...
all lines. Use `--lint-scope all` to audit existing code.

`RUST_LLM_TIDY_DIFF_BASE` supplies the baseline unless `--diff-base` is set.
Without paths, an explicit baseline selects allowed files recursively from
the current directory instead of the tracked-change fallback.

Missing Git context skips changed-line findings with a warning. Invalid
explicit baselines fail; neither case silently widens reporting.
See [reporting scope] for scope precedence and remapping limitations.
