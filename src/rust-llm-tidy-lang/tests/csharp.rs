...
fn indexed_lints_should_follow_resolvable_cross_file_chains() {
    ...
    for (label, caller, helper, end, expected) in cases {
        ...
        let index = rust_llm_tidy_lang::backends::CanThrowIndex::from_parses(&parses);
...
fn indexed_lints_should_merge_separately_supplied_callers() {
    for (source, foreign_source) in [
        ...
        let foreign = rust_llm_tidy_lang::backends::CanThrowIndex::from_parses([&helper]);
        let complete = rust_llm_tidy_lang::backends::CanThrowIndex::from_parses([&caller, &helper]);
...
fn indexed_lints_should_respect_exception_tags() {
    ...
    for (label, tag, expected) in [
        ...
        let index = rust_llm_tidy_lang::backends::CanThrowIndex::from_parses([&caller, &helper]);
