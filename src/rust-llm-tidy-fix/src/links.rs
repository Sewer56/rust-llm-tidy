...
pub fn fix_links(input: &str) -> Cow<'_, str> {
    ...
    for segment in &segments {
        if step_fence(&mut fence_stack, segment) {
            continue;
        }
    ...
    // lazily on the first changed segment (mirrors `fix_fences`).
...
/// Update the open-fence stack for `segment` and report whether it is a fence
/// delimiter line. Reuses the byte-exact [`crate::fences::parse_fence`], so
/// fence skipping stays in lock-step with `fix_fences`.
fn step_fence(stack: &mut Vec<(char, usize)>, segment: &str) -> bool {
    let (content, _) = split_terminator(segment);
    let (_, body) = strip_doc_prefix(content);
...
fn rewrite_links(
    ...
    while i < bytes.len() {
            ...
            && let Some((text, url, end)) = parse_inline_link(body, i)
        {
            ...
            // Not hoisted: copy the inline link verbatim and advance.
